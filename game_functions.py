...
def uppdate_screen(ai_settings, screen, stats, sb, ship, aliens, bullets,
	              play_button):
	"""更新屏幕上的图像，屏切换到新屏幕"""
	# 每次循环时都重绘屏幕
...
def	check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets):
	...
	if len(aliens) == 0:
		#删除现有的子弹并新建一群外星人,如果外星人全部被消灭，提升一个等级
		bullets.empty()
...
def ship_hit(ai_settings, stats, sb, screen, ship,
			 aliens, bullets):
	"""响应被外星人撞到的飞船"""
...
def check_high_score(stats, sb):
	...
	if stats.score > int(high_score):
		...
		with open(filename, 'w') as f_obj:
			json.dump(str(high_score), f_obj)
